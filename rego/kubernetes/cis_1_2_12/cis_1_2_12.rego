...
violation[msg]{
    ...
    msg = sprintf("%v Ensure that the admission control plugin AlwaysPullImages is set (Manual)",[container.name])
...
argus_judement(commandArgs) = "allow"{
   ...
   not args_contains_string(commandArgs,"--enable-access-plugins","AlwaysPullImages")
}else = "notallow"

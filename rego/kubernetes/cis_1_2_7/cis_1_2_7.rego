...
violation[msg]{
    ...
    msg = sprintf("%v Ensure that the --authorization-mode argument is not set to AlwaysAllow (Automated)",[container.name])
...
argus_judement(commandArgs) = "allow"{
   ...
   args_contains_string(commandArgs,"--authorization-mode","AlwaysAllow")
}else = "notallow"

...
violation[msg]{
    ...
    not args_contains_string(commandArgs,"--enable-admission-plugins","SecurityContextDeny")
    msg = sprintf("%v Ensure that the admission control plugin SecurityContextDeny is set if PodSecurityPolicy is not used (Manual)",[container.name])
